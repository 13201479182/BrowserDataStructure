...
let count = 1;
describe(`heap:complex test combined usage`, () => {
    /**
     ...
     * 4. 查看结果集是否满足预期
     */
    test(`${count++}. 测试大堆插入元素&响应式处理为小堆&批量插入元素`, () => {
        const heap = new Heap();
        expect(
            heap
                .insertElement(1)
     ...
     * 5. 查看结果集是否满足预期
     */
    test(`${count++}. 测试小堆插入元素&响应式处理为大堆&批量插入元素&排序`, () => {
     ...
     * 4. 查看结果集是否满足预期
     */
    test(`${count++}. 测试大堆object[]插入元素&响应式处理为小堆&响应是处理priority&批量插入元素`, () => {
